...
class FeedCardEvent extends React.Component {
  ...
  componentDidMount() {
    ...
    actions.fetchHomeFeedEventData(profile.data.id, offset, limit, this.props.feedType);
  }
      ...
      if (nextData && (!nextData.equals(previousData) || this.feedCardArray.length === 0)) {
        if (!nextData.isEmpty()) {
          ...
          _.each(tempPropsArray, (props, index) => {
            this.eventCount++;
              if(props.body.type === this.props.feedType){
                this.feedCardArray.push(<div key={props.body.type + index + this.eventCount}><PageFeedCard {...props} {...props.body} {...this.props}/></div>)      
              }
            })
            }
  ...
  loadMoreFeedData(e) {
    ...
    if(cardLength >= 10 * this.cardCount) {
      ...
      const limit = 10;
      actions.fetchHomeFeedEventData(profile.data.id, offset, limit, self.props.feedType);
    }  else {
      let { apiActions, dispatch } = this.props
      dispatch(apiActions.setHasMoreEventFeedData(false));
    }
  }
  render() {
    let self = this
    ...
    return (
      ...
      hasMore={this.props.hasMoreEvent}
...
function mapStateToProps(state, props) {
    return {
        ...
        hasMoreEvent : state.get('homePage').get('hasMoreEvent')
    }
}

export default connect(mapStateToProps)(FeedCardEvent)
