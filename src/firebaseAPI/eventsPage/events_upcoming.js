...
export function getUpcomingEvents() {
  return new Promise((resolve, reject) => {
    ...
    firebaseDB.child('views/events_page/body_upcoming_events').orderByChild('weight').limitToLast(3).once('value').then(function (eventListSnap) {
      const upcomingEvents_promises = [];
      eventListSnap.forEach(function (individualEvent) {
        const eventKey = individualEvent.key;
        const upcomingEventpromise = firebaseDB.child('/resources/' + eventKey).once('value').then(function (eventSnap) {
        ...
        upcomingEvents_promises.push(upcomingEventpromise);
      });
      return Promise.all(upcomingEvents_promises);
    }).then(function (values) {
      resolve(values);
    });
  });
