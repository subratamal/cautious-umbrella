...
export function getOpportunitiesFullTime() {
  return new Promise((resolve, reject) => {
    firebaseDB.child('resources/').orderByChild('subtype').equalTo('Full time job').once('value')
    .then((opportunitiesSnap) => {
      ...
      for (const key in opportunitiesObject) {
        if ({}.hasOwnProperty.call(opportunitiesObject, key) && opportunitiesObject[key].subtype === 'Full time job') {
      ...
      let count = opportunitiesPageCount.fullTimeCount;
      if (objectsArray.length < opportunitiesPageCount.fullTimeCount) {
        count = objectsArray.length;
      }
      fullTimeArray = objectsArray.slice(0, count);

      fullTimeArray.forEach((opportunityObj) => {
