...
/* eslint-disable */
export function getOpportunitiesScholarships() {
/* eslint-enable */
  return new Promise((resolve, reject) => {
    firebaseDB.child('resources/').orderByChild('subtype').equalTo('Scholarship').once('value')
    .then((opportunitiesSnap) => {
      ...
      /* eslint-enable */
        if ({}.hasOwnProperty.call(opportunitiesObject, key) && opportunitiesObject[key].subtype === 'Scholarship') {
          objectsArray.push(opportunitiesObject[key]);
        }
      }

      objectsArray.sort((a, b) => parseInt(b.updated_time, 10) - parseInt(a.updated_time, 10));
      let count = opportunitiesPageCount.scholarshipsCount;
      if (objectsArray.length < opportunitiesPageCount.scholarshipsCount) {
        count = objectsArray.length;
      }
      scholarshipsArray = objectsArray.slice(0, count);

      scholarshipsArray.forEach((opportunityObj) => {
